...
function buildStatusSelect(task, className = "board-status") {
  const selectedStatus = TASK_STATUS_OPTIONS.includes(task.status) ? task.status : "פתוחה";
  return `<select class="${className} ${getStatusBadgeClass(selectedStatus)}" data-status-select="true" aria-label="סטטוס משימה">${TASK_STATUS_OPTIONS.map((status) => `<option value="${status}" ${selectedStatus === status ? "selected" : ""}>${status}</option>`).join("")}</select>`;
}
